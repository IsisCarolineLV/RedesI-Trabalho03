...
public class MeioDeComunicacao extends Thread {
  ...
  public void setTransmissor(CamadaFisicaTransmissora camada_Fisica_Transmissora) {
  ...
  public synchronized void comunicar(int[] fluxoBrutoDeBits, boolean ehTransmissor) {
    filaMensagens.add(fluxoBrutoDeBits);
   ...
   * mensagem chega, determina a direcao e chama o metodo 'run' 
   * correspondente.
   * Parametros: vazio
   * Retorno: vazio
   ****************************************************************/
  public void run() {
    while (!interrompido) {
      ...
      ehTransmissor = filaEmissores.poll();
      if (ehTransmissor) {
        runTransmissor();
      } else {
        runReceptor();
  ...
  public void runTransmissor() {
    ...
    boolean erro = gerarErro(); // vai haver erro na transmissao desse quadro?
    int localErro = 0;
    ...
    if (erro && escopoDoQuadro > 0) {
      ...
      if (tipoDeEnquadramento == 3)
        camada_Fisica_Receptora.avisaErro();
    }
  ...
  /* ***************************************************************
   * Metodo: runReceptor
   * Funcao: Simula a transmissao do Receptor para o Transmissor
   * (envio de ACKs). Funciona de forma similar ao runTransmissor,
   * mas move o sinal visual na direcao oposta e para simplificar 
   * supus que no envio do ack nao da errado nunca. No final, entrega 
   * o ACK ao transmissor.
   * Parametros: vazio
   * Retorno: vazio
   ****************************************************************/
  public void runReceptor() {
    barraDeProgresso.setScaleX(-1);
    int[] fluxoBrutoDeBitsReceptor = fluxoBrutoDeBits;
     ...
     * int localErro = 0;
     * int escopoDoQuadro=
     * (totalBitsValidos-bitCont<24)?(totalBitsValidos-bitCont):24;
     * if(erro && escopoDoQuadro>0){
     * localErro = bitCont+(new Random()).nextInt(escopoDoQuadro);
     * }
     */
    ...
    for (int i = 0; i < tam; i++) {
      ...
      for (int j = 31; j >= 0; j--) {
        ...
        if (i == tam - 1 && j <= 31 - (tamanhoByte * bytesPreechidos)) {
          ...
          boolean comparacao = (mascara & fluxoBrutoDeBitsReceptor[i]) != 0;
          // se estamos no local que foi sorteado para dar erro e houve um erro inverte o
          // valor verdade de comparacao (ou seja, se ia colocar um 1 coloca um 0 e
          // vice-versa)
          /*
      ...
      fluxoBrutoDeBitsTransmissor[i] = inteiroCopia;
    ...
    barraDeProgresso.setScaleX(1);
    porcentagem = 0;
    camada_Fisica_Transmissora.camadaFisicaReceptora(fluxoBrutoDeBitsReceptor);
  }
